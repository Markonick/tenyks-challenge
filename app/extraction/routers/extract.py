...
async def post_dataset(request: TenyksExtractionRequest) -> TenyksResponse:
    ...
    images_path = f"{backend_base_url}/images"
    images_request = TenyksImagesRequest(dataset_id=dataset_id)
    # images_response = await get_async_request_handler(url=images_path, request=images_request)
    
    # session = aioboto3.Session()
    # async with session.resource("s3") as s3:
    #     bucket = await s3.Bucket('testbucket')  # <----------------
