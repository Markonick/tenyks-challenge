--
-- Database schema for Mia IQ (technical recall)
--

-- Text columns that have some intrinsic length (e.g. laterality is always a single letter L or R)
-- are of type VARCHAR(n) where n > 0 and typically n << 64. Description columns that have no
-- intrinsic length limit are of type TEXT.
CREATE SCHEMA IF NOT EXISTS miaiq;

CREATE DOMAIN miaiq.uuid_type VARCHAR(64) NOT NULL;
COMMENT ON DOMAIN miaiq.uuid_type IS 'DICOM UUIDs have a maximum length of 64 characters, as set in the DICOM standard.';

CREATE DOMAIN miaiq.name_type VARCHAR(128) NOT NULL;
COMMENT ON DOMAIN miaiq.name_type IS 'Names typically shown in the front-end as a (localized) label.';

CREATE DOMAIN miaiq.url_type VARCHAR(1024);
COMMENT ON DOMAIN miaiq.url_type IS 'Permalink URLs (e.g. AWS S3 URLs, Amazon Blob URLs).';


CREATE TABLE IF NOT EXISTS miaiq.sop_class(
...
INSERT INTO miaiq.sop_class (id, uuid, name, description)
VALUES
...
CREATE TABLE IF NOT EXISTS miaiq.model(
    id INT GENERATED BY DEFAULT AS IDENTITY,
    name miaiq.name_type,
    criteria_set_id INT NOT NULL,
...
CREATE TABLE IF NOT EXISTS miaiq.organization(
    id INT GENERATED BY DEFAULT AS IDENTITY,
    name miaiq.name_type,
    description TEXT,
    PRIMARY KEY (id),
...
COMMENT ON TABLE miaiq.organization IS 'A broader organization where the technical recall project is used. In practice this means that all users from an organization can see data that belongs to it.';


CREATE TABLE IF NOT EXISTS miaiq.user_role(
    id INT GENERATED BY DEFAULT AS IDENTITY,
    name miaiq.name_type,
    description TEXT,
    PRIMARY KEY (id),
...
COMMENT ON TABLE miaiq.user_role IS 'Represents a MiaIQ user role such as a director, radiographer or an auditor';


INSERT INTO miaiq.user_role(id, name, description)
VALUES
    (1, 'Director', 'Director of screeing department'),
    (2, 'Auditor', 'Member of the auditing staff, responsible for reviewing cases'),
    (3, 'Radiographer', 'Technician (radiographer), trained to take mammo X-rays')
ON CONFLICT (id) DO NOTHING;


CREATE TABLE IF NOT EXISTS miaiq.audit_user(
    id INT GENERATED BY DEFAULT AS IDENTITY,
    ...
    CONSTRAINT fk_miaiq_user_organization FOREIGN KEY (organization_id) REFERENCES miaiq.organization(id)
);
COMMENT ON TABLE miaiq.audit_user IS 'Represents a user authenticated with OIDC.';
COMMENT ON COLUMN miaiq.audit_user.organization_id IS 'The broader organization the user belongs to.';
COMMENT ON COLUMN miaiq.audit_user.name IS 'User name consistent with OIDC claims.';
